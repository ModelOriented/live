...
Functions presented in this vignette use simplified and improved interface introduced in version 1.5.3 Older versions of functions (without the "2" suffix) were kept for consistency with the examples given in https://arxiv.org/abs/1804.01955.
New interface is simpler (less arguments, more defaults), richer (two methods of sampling and optionally some variables can be excluded from sampling) and allows for using pipe operator.
...
Now we can visualize it with the `plot_explanation2` function.
