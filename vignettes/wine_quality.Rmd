...
While the object created by `sample_locally` function stores the dataset, explained instance and the name of the response variable, object returned by `add_predictions2` function also stores the fitted black box model.
The result of applying `sample_locally` functions doesn't contain the response and the result of `add_predictions2` contains a column with model predictions, which has the same name as response in original dataset.
