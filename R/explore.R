...
replace_items_by_rows <- function(chosen_vars, data, target) {
  lapply(seq_along(chosen_vars), function(x) {
    row <- target[x, ]
    column_to_sample <- unlist(data[, chosen_vars[x]], use.names = FALSE)
    column_to_sample <- column_to_sample[!is.na(column_to_sample)]
    row[1, chosen_vars[x]] <- sample(column_to_sample, 1)
    row
  }) %>%
    dplyr::bind_rows()
}
...
check_for_na <- function(data, explained_instance) {
  if(any(is.na(unlist(explained_instance)))) warning("Missing values present in explained instance.")
